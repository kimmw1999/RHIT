...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(1,n):
        a=rect.get_lower_left_corner().x
        b=rect.get_lower_left_corner().y
        c=rect.get_upper_right_corner().x
        d=rect.get_upper_right_corner().y

        point1=rg.Point(a-delta*k,b+delta*k)
        point2=rg.Point(c+delta*k, d-delta*k)
