...
def hourglass(window, n, point, radius, color):
    ...
    line=rg.Line(rg.Point(center.x-circle.radius, center.y),
                 rg.Point(center.x+circle.radius, center.y))
    for k in range(n):
        newCircle=circle.clone()
        newCircle.fill_color=color
        newCircle.move_by(-radius*k, -math.sqrt(3)*radius*k)
        newLine=line.clone()
        newLine.move_by(-radius*k, -math.sqrt(3)*radius*k)
        newCircle.attach_to(window)
        newLine.attach_to(window)

        for j in range(1, k+1):
            anotherCircle=newCircle.clone()
            anotherCircle.fill_color=color
            anotherCircle.move_by(2*radius*j, 0)
            anotherLine=newLine.clone()
            anotherLine.move_by(2*radius*j, 0)
            anotherCircle.attach_to(window)
            anotherLine.attach_to(window)
        window.render()
...
def many_hourglasses(window, square, m, colors):
    ...
    newCenter=center.clone()
    radius = square.length_of_each_side / 2
    hourglass(window, 1, square.center, radius, colors[0])

    for k in range(1, m):
        j=k%len(colors)
        newCenter.move_by((2*k+1)*radius,0)
        rectangle=rg.Rectangle(rg.Point(newCenter.x-((1+k)*radius),
                                        newCenter.y-math.sqrt(3)*(k*radius)-radius),
                               rg.Point(newCenter.x + ((1 +  k) * radius),
                                        newCenter.y + math.sqrt(3) * (k * radius) + radius
                                        ))
        rectangle.attach_to(window)
        window.render()
        hourglass(window, k+1, newCenter, radius, colors[j])
