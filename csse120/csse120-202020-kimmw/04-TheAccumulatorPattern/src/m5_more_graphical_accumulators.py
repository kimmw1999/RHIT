...
def draw_squares_from_circle(n, circle, window):
    ...
    # -------------------------------------------------------------------------
    rad=circle.radius
    x_cor=circle.center.x
    y_cor=circle.center.y
    circle.attach_to(window)
    window.render()
    for k in range(n):
        rectangle=rg.Rectangle( rg.Point( x_cor - rad + k* rad , y_cor - rad + k*rad), rg.Point( x_cor + rad + k*rad , y_cor + rad + k*rad ) )
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    cent_x=rectangle.get_center().x
    cent_y=rectangle.get_center().y

    for k in range(m):
        x_cor= cent_x- (width/2)- height/2 - 2*k*(height/2)
        y_cor= cent_y
        circle=rg.Circle( rg.Point(x_cor, y_cor), height/2)
    ...
    for k in range(n):
        x_cor= cent_x
        y_cor= rectangle.get_upper_right_corner().y - (width/2) - width*k
        circle=rg.Circle( rg.Point(x_cor, y_cor), width/2)
        circle.attach_to(window)
        window.render()
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    cent1=rectangle1.get_center()
    cent2=rectangle2.get_center()
    change_x= cent1.x-rectangle1.get_lower_left_corner().x
    change_y=rectangle1.get_lower_left_corner().y-  cent1.y
    for k in range(n):
        line=rg.Line(rg.Point(cent1.x- change_x*k, cent1.y+ change_y*k), rg.Point(cent2.x- change_x*k, cent2.y+change_y*k))
        line.thickness=5
        if(k%2==0):
