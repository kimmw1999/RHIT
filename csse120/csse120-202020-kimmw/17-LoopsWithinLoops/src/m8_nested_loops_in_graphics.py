...
def draw_L(window, circle, r, c):
    ...
    for k in range(r+3):
        for j in range(3):
            ...
            circle2.move_by(j*2*circle.radius, k*2*circle.radius)
            circle2.attach_to(window)
    for m in range(c):
        for n in range(3):
            circle3= rg.Circle(circle2.center, circle2.radius)
            circle3.fill_color=circle2.fill_color
            circle3.move_by((m+1)*2*circle2.radius, -n*2*circle2.radius)
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range(1,n+1):
        for j in range(k):
            rectangle2=rg.Rectangle(rectangle.get_lower_left_corner(), rectangle.get_upper_right_corner())
            rectangle2.move_by(-rectangle.get_width()*j, rectangle.get_height()*k)
