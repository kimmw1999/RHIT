...
def run_test_remote_control(robot):
    ...
    while True:
        ...
        if robot.remote_control.is_pressed(1, "red_up") == True:
            robot.drive_system.left_motor.turn_on(speed)
        elif robot.remote_control.is_pressed(1, "red_down") == True:
            robot.drive_system.left_motor.turn_on(-speed)
        else:
            robot.drive_system.left_motor.turn_off()
        if robot.remote_control.is_pressed(1, "blue_up") == True:
            robot.drive_system.right_motor.turn_on(speed)
        elif robot.remote_control.is_pressed(1, "blue_down") == True:
            robot.drive_system.right_motor.turn_on(-speed)
        else:
            robot.drive_system.right_motor.turn_off()

        if robot.remote_control.is_pressed(2, "red_up") == True:
            robot.arm_and_claw.raise_arm()
        elif robot.remote_control.is_pressed(2, "red_down") == True:
            robot.arm_and_claw.lower_arm()
        if robot.remote_control.is_pressed(2, "blue_up") == True:
            robot.arm_and_claw.calibrate_arm()
        if robot.brick_buttons.is_backspace_pressed() == True:
            break
